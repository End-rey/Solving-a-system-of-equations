...
public class App {
    ...
    static public double[] LUP(double[][] a, double[] b) {
        int n = a.length;
    ...
    static public double[] QR(double[][] a, double[] b) {
        int n = a.length;
        ArrayList<double[][]> QR = matrix.QandR(a);
    ...
    public static Pair simple_iteration(double[][] a, double[] b1, double e) {
        ...
        double u = 1 / matrix.norma2(A);
        double[][] B = matrix.sum(E, matrix.mult(A, -u));

        double[] c = matrix.mult(b, u);

        double[] xk = new double[n];

        double[] xk1 = matrix.copy(c);

        while ((matrix.norma2(B) / (1 - matrix.norma2(B))) * matrix.norma(matrix.sum(xk1, matrix.mult(xk, -1))) > e) {
            count++;
            xk = matrix.copy(xk1);
            xk1 = matrix.sum(matrix.mult(B, xk), c);
        }

        Pair hm = new Pair(xk1, count);
