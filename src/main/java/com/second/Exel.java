...
public class Exel {
    ...
    public static void init() throws IOException {
        try (Workbook wb = new HSSFWorkbook()) {
            sheet = wb.createSheet("Листочек1");
            sheet1 = wb.createSheet("Листочек2");
            myStyle = (HSSFCellStyle) wb.createCellStyle(); // создаем стиль ячейки

            // создаем границы ячейки в стиле
            ...
            myStyle.setAlignment(HorizontalAlignment.CENTER); // стиль выравнивания по горизонтали
            myStyle.setVerticalAlignment(VerticalAlignment.CENTER); // стиль выравивание по вертикали
    ...
    private static void first_line() {
        ...
        cell1[1].setCellValue("x^");
        cell1[2].setCellValue("e");
    ...
    private static void first_line1() {
        ...
        cell1[2].setCellValue("э");
        cell1[3].setCellValue("x^");
        cell1[4].setCellValue("e");
    ...
    private static void test(double[][] a, double[] b) {
        ...
        double[] x1 = matrix.copy(Test.getRes()[testn]);

        Pair[] res_simpIter = new Pair[4];
        for (int i = 0; i < 4; i++) {
            res_simpIter[i] = App.simple_iteration(a, b, e[i]);
        }

        Pair[] res_zeid = new Pair[4];
        for (int i = 0; i < 4; i++) {
            res_zeid[i] = App.Zeidel(a, b, e[i]);
        }

        double[] res_lup = App.LUP(a, b);

        double[] res_qr = App.QR(a, b);

        Row[] row = new Row[4];
        for (int i = 0; i < 4; i++) {
            row[i] = sheet.createRow(i + 2 + 4 * testn);
            row[i].setHeightInPoints(25);
        }

        sheet.addMergedRegion(new CellRangeAddress(2 + 4 * testn, 5 + 4 * testn, 0, 0));
        sheet.addMergedRegion(new CellRangeAddress(2 + 4 * testn, 5 + 4 * testn, 9, 9));
        sheet.addMergedRegion(new CellRangeAddress(2 + 4 * testn, 5 + 4 * testn, 1, 1));
        sheet.addMergedRegion(new CellRangeAddress(2 + 4 * testn, 5 + 4 * testn, 10, 10));
        sheet.addMergedRegion(new CellRangeAddress(2 + 4 * testn, 5 + 4 * testn, 11, 11));
        sheet.addMergedRegion(new CellRangeAddress(2 + 4 * testn, 5 + 4 * testn, 12, 12));
            ...
            for (int j = 0; j < 13; j++) {
                ...
                switch (j) {
                    ...
                    case 3: {
                        String temp = "";
                        for (int k = 0; k < n; k++)
                            temp += res_simpIter[i].getX()[k] + ", ";
                        cell[i][j].setCellValue(temp);
                        break;
                    }
                    case 4: {

                        cell[i][j].setCellValue(matrix.norma(matrix.sum(res_simpIter[i].getX(), x1)));
                        break;
                    }
                    case 5: {
                        cell[i][j].setCellValue(res_simpIter[i].getCount());
                        break;
                    }
                    case 6: {
                        String temp = "";
                        for (int k = 0; k < n; k++)
                            temp += res_zeid[i].getX()[k] + ", ";
                        cell[i][j].setCellValue(temp);
                        break;
                    }
                    case 7: {

                        cell[i][j].setCellValue(matrix.norma(matrix.sum(res_zeid[i].getX(), x1)));
                        break;
                    }
                    case 8: {
                        cell[i][j].setCellValue(res_zeid[i].getCount());
                        break;
                    }
                    case 9: {
                        String temp = "";
                    ...
                    case 10: {
                        ...
                        cell[i][j].setCellValue(matrix.norma(matrix.sum(res_lup, x1)));
                        break;
                    }
                    case 11: {
                        String temp = "";
                    ...
                    case 12: {
                        ...
                        cell[i][j].setCellValue(matrix.norma(matrix.sum(res_qr, x1)));
                        break;
                    }
                    default:
                        break;
    ...
    private static void test5() {
        ...
        Pair[][][] res_simpIter = new Pair[4][2][4];
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 2; j++) {
                for (int j2 = 0; j2 < 4; j2++) {
                    res_simpIter[i][j][j2] = App.simple_iteration(test5[i][j].getMatr(), test5[i][j].getVect(), e[j2]);
                }
            }
        }

        Pair[][][] res_zeid = new Pair[4][2][4];
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 2; j++) {
                for (int j2 = 0; j2 < 4; j2++) {
                    res_zeid[i][j][j2] = App.Zeidel(test5[i][j].getMatr(), test5[i][j].getVect(), e[j2]);
                }
            }
        }

        double[][][] res_lup = new double[4][2][4];
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 2; j++) {
                res_lup[i][j] = App.LUP(test5[i][j].getMatr(), test5[i][j].getVect());
            }
        }

        double[][][] res_qr = new double[4][2][4];
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 2; j++) {
                res_qr[i][j] = App.QR(test5[i][j].getMatr(), test5[i][j].getVect());
            }
        }
        ...
        for (int i = 0; i < 32; i++) {
            for (int j = 0; j < 15; j++) {
                ...
                cell[i][j].setCellStyle(myStyle);
                int n = i / 8;
                int ee = ((0 <= i & i < 4) || (8 <= i & i < 12) || (16 <= i & i < 20) || (24 <= i & i < 28)) ? 0 : 1;
                switch (j) {
                    case 1: {
                        if ((i) % 8 == 0)
                    ...
                    case 5: {
                        String temp = "" + res_simpIter[n][ee][i % 4].getX()[0];

                        for (int k = 1; k < n + from_n; k++)
                            temp += ", " + res_simpIter[n][ee][i % 4].getX()[k];
                        cell[i][j].setCellValue(temp);
                        break;
                    }
                    case 6: {
                        ...
                        x1[n + from_n - 1] = -1;
                        cell[i][j].setCellValue(matrix.norma(matrix.sum(res_simpIter[n][ee][i % 4].getX(), x1)));
                        break;
                    }
                    case 7: {
                        cell[i][j].setCellValue(res_simpIter[n][ee][i % 4].getCount());
                        break;
                    }
                    case 8: {
                        String temp = "" + res_zeid[n][ee][i % 4].getX()[0];
                        for (int k = 1; k < n + from_n; k++)
                            temp += ", " + res_zeid[n][ee][i % 4].getX()[k];
                        cell[i][j].setCellValue(temp);
                        break;
                    }
                    case 9: {
                        double[] x1 = new double[n + from_n];
                        x1[n + from_n - 1] = -1;
                        cell[i][j].setCellValue(matrix.norma(matrix.sum(res_zeid[n][ee][i % 4].getX(), x1)));
                        break;
                    }
                    case 10: {
                        cell[i][j].setCellValue(res_zeid[n][ee][i % 4].getCount());
                        break;
                    }
                    case 11: {
                        String temp = "" + res_lup[n][ee][0];
                        for (int k = 1; k < n + from_n; k++)
                            temp += ", " + res_lup[n][ee][k];
                        cell[i][j].setCellValue(temp);
                        break;
                    ...
                    case 12: {
                        double[] x1 = new double[n + from_n];
                        x1[n + from_n - 1] = -1;
                        cell[i][j].setCellValue(matrix.norma(matrix.sum(res_lup[n][ee], x1)));
                        break;
                    }
                    case 13: {
                        String temp = "" + res_qr[n][ee][0];
                        for (int k = 1; k < n + from_n; k++)
                            temp += ", " + res_qr[n][ee][k];
                        cell[i][j].setCellValue(temp);
                        break;
                    ...
                    case 14: {
                        double[] x1 = new double[n + from_n];
                        x1[n + from_n - 1] = -1;
                        cell[i][j].setCellValue(matrix.norma(matrix.sum(res_qr[n][ee], x1)));
                        break;
                    }
                    default:
                        break;
